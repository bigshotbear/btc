...
from btc_engine.storage.schema import ensure_schema, get_missing_tables
...
async def lifespan(_: FastAPI):
    # Repair early deployments that were stamped at an Alembic revision before
    # all collector tables existed. This operation is idempotent and does not
...
async def healthz():
    try:
        ...
        missing_tables = await get_missing_tables(engine)
        if missing_tables:
            return JSONResponse(
                status_code=503,
                content={
                    "status": "error",
                    "database": "connected_but_schema_incomplete",
                    "missing_tables": missing_tables,
                    "hint": "Redeploy the latest revision so migration 0002 can repair the schema.",
                },
            )
        return {
            "status": "ok",
