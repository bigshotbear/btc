...
class Settings(BaseSettings):
    ...
    run_collectors_in_api: bool = False
    ...
    def normalize_database_url(cls, value: str) -> str:
        """Force PostgreSQL URLs onto SQLAlchemy's asyncpg driver.

        Hosting platforms and copied connection strings may use plain
        ``postgresql://`` or explicitly name a synchronous driver such as
        ``postgresql+psycopg2://``.  The application and Alembic environment
        are asynchronous, so all supported PostgreSQL variants must be
        normalized before SQLAlchemy creates an engine.
        """
